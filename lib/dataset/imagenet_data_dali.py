...
class HybridTrainPipe(Pipeline):
    def __init__(self, batch_size, world_size, device_id, data_dir, 
        ...
        super(HybridTrainPipe, self).__init__(batch_size,
                                                ...
                                                device_id,
                                                seed=12 + device_id)
        self.input = ops.MXNetReader(path = osp.join(data_dir + '/rec', data_cfg.train_pref+".rec"),
                                    index_path = osp.join(data_dir + '/rec', data_cfg.train_pref+".idx"),
                                    random_shuffle = True, 
                                    shard_id = device_id, 
...
#                                             device_id,
#                                             seed=12 + device_id)
#         self.input = ops.MXNetReader(path = osp.join(data_dir + '/rec', data_cfg.val_pref+".rec"),
#                                     index_path = osp.join(data_dir + '/rec', data_cfg.val_pref+".idx"),
...
def get_train_loader(batch_size, world_size, device_id, 
    ...
    train_loader = DALIClassificationIterator_(pipe, reader_name="Reader", 
                fill_last_batch=True, auto_reset=True)
    return train_loader
...
def get_single_loader(data_cfg, world_size, device_id, data_dir):
    val_loader = get_val_loader(data_cfg.batch_size,
                                world_size, device_id,
                                data_dir, data_cfg)
    return  val_loader