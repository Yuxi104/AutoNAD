...
from supernet_engine import train_one_epoch_lat, evaluate_lat
...
def main(args):
    ...
    if args.resume:
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            ...
            # lat_optimizer.load_state_dict(checkpoint['lat_optimizer'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_lat(
            model, history_list, data_loader_train, optimizer, 
            device, epoch, loss_scaler,
            args.clip_grad, model_ema, mixup_fn, # keep in eval mode during finetuning
            amp=args.amp, teacher_model=teacher_model,
            teach_loss=teacher_loss,choices=choices, mode=args.mode,retrain_config=retrain_config,
            num_classes=args.nb_classes)
        
        lr_scheduler.step(epoch)
        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint_tiny.pth']
        ...
        test_stats = evaluate_lat(data_loader_val, model, device, args.nb_classes, 
                              choices, mode=args.mode, retrain_config=retrain_config)
        val_info = str(test_stats)
        print(val_info)
        ...
        if args.output_dir and utils.is_main_process():
            with (output_dir / results_file).open("a") as f:
    ...
    # save the latency prior
    if utils.is_main_process():
        with open(f'{args.output_dir}/train_history.json', 'w') as f:
